    ...
    <nav class="flex items-center justify-between flex-wrap bg-teal-400 p-6">
      ...
      <div class="w-full block flex-grow lg:flex lg:items-center lg:w-auto">
        <div class="text-sm lg:flex-grow">
          <a onClick={() => props.changeView('chooseView')} class="block mt-4 lg:inline-block lg:mt-0 text-teal-200 hover:text-white mr-4">
            wishNwell
                </a>
        </div>
        <div class="flex space-x-2">
          <a onClick={props.signOut} class="inline-block text-sm px-4 py-2 pl-4 leading-none border rounded text-white border-white hover:border-transparent hover:text-teal-500 hover:bg-white mt-4 lg:mt-0">Sign Out</a>
        </div>
      </div>
    </nav>
  )
